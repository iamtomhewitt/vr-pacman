...
{
    public AudioSource  ghostCanBeEaten, ghostHasBeenEaten, ghostMoving, ghostRunningHome;
    ...
    void Update()
    {
        for (int i = 0; i < ghosts.Length; i++)
        {
                ...
                !ghosts[3].GetComponent<Ghost>().canBeEaten)
            {
                ...
                if (!pacman.GetComponent<Pacman>().dead && gameManager.canStartGame && gameManager.numberOfFood > 0)
                {
